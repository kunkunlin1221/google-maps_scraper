...
GM_WEBPAGE = 'https://www.google.com.tw/maps/'
MAX_WAIT = 10
MAX_RETRY = 5
MAX_SCROLLS = 40
...
class GoogleMapsScraper:
        ...
        for i, search_point_url in enumerate(search_point_url_list):
            ...
            if (i+1) % 10 == 0:
                ...
                df_places = df_places[['search_point_url', 'href', 'name',
                                       'rating', 'num_reviews', 'close_time', 'other']]
    ...
    def __parse(self, review):
        ...
        item['retrieval_date'] = datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')
        item['rating'] = rating
    ...
    def __parse_place(self, response, url):
        ...
        try:
            place['overall_rating'] = float(response.find('div', class_='F7nice ').find(
                'span', class_='ceNzKf')['aria-label'].split(' ')[1])
        except Exception as e:
            place['overall_rating'] = None

        try:
            place['n_reviews'] = int(response.find('div', class_='F7nice ').text.split(
                '(')[1].replace(',', '').replace(')', ''))
        except Exception as e:
            place['n_reviews'] = 0

        try:
            place['n_photos'] = int(response.find('div', class_='YkuOqf').text.replace(
                '.', '').replace(',', '').split(' ')[0])
        except Exception as e:
            place['n_photos'] = 0
